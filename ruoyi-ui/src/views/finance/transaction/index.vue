  ...
  methods: {
    ...
    getList() {
      ...
      listTransaction(this.queryParams).then(response => {
        ...
        Notification.error({
          title: '请求失败',
          message: '获取交易数据失败，请检查后台接口是否正常工作。'
        });
        this.loading = false;
      });
    },
    ...
    getCategoryOptions() {
      ...
      listCategoryByType(1).then(response => {
        ...
        listCategoryByType(2).then(response => {
          ...
          Notification.error({
            title: '请求失败',
            message: '获取收入分类数据失败，请检查后台接口是否正常工作。'
          });
        });
      }).catch(error => {
        Notification.error({
          title: '请求失败',
          message: '获取支出分类数据失败，请检查后台接口是否正常工作。'
        });
      });
    },
    // 类型发生变化时，更新可选分类
