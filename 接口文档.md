...
```json
{
  ...
  "data": {
    ...
    "user": {
      ...
      "role": "admin",
      "avatar": "/profile/avatar/2023/08/10/avatar_20230810120530.jpg",
...
```json
{
  "userId": 1,             // 用户ID（必填）
  "nickname": "张三",      // 用户昵称（必填）
  "email": "user@example.com", // 电子邮箱（必填）
  "phone": "13800138000",  // 手机号码（必填）
  "gender": 1,             // 性别（0:未知 1:男 2:女）（选填）
  "wechat": "wx123456",    // 微信号（选填）
  "qq": "123456",          // QQ号（选填）
  "signature": "个性签名"  // 个性签名（选填）
}
```
...
| 参数名     | 类型 | 必填 | 描述                                                   |
| ---------- | ---- | ---- | ------------------------------------------------------ |
| avatarfile | File | 是   | 头像图片文件，仅支持jpg、jpeg、png、gif、bmp等图片格式 |
| userId     | Long | 否   | 用户ID，不传则默认为当前登录用户                       |
...
```json
{
  "code": 200,
  "msg": "头像上传成功",
  "imgUrl": "/profile/avatar/2023/08/10/avatar_20230810120530.jpg"
}
```

- **注意事项**:
  1. 文件大小限制为10MB
  2. 仅支持图片格式（jpg、jpeg、png、gif、bmp）
  3. 上传的图片会自动生成唯一文件名并保存到系统配置的头像路径下
  4. 文件路径格式为：`/profile/avatar/年/月/日/文件名.扩展名`
  5. 系统会将图片URL保存到用户表的avatar字段中，并在返回用户信息时包含此字段
...
| 参数名 | 类型 | 必填 | 描述 |
|-------|------|------|------|
| userId | Long | 是 | 用户ID（如不提供则使用当前登录用户ID，非管理员用户只能查询自己的交易记录） |
...
| 参数名 | 类型 | 必填 | 描述 |
|-------|------|------|------|
| file | File | 是 | 票据图片文件，仅支持jpg、jpeg、png等图片格式 |
| transactionId | Long | 否 | 交易记录ID，不传则只上传不关联交易记录 |
...
```json
{
  ...
  "msg": "票据上传成功",
  "data": {
    "imageUrl": "/profile/transaction/2023/08/10/receipt_20230810120530.jpg"
  }
}
```
...
- **注意事项**:
  1. 文件大小限制为5MB
  2. 仅支持图片格式（jpg、jpeg、png）
  3. 上传的图片会自动生成唯一文件名并保存到系统配置的交易票据路径下
  4. 文件路径格式为：`/profile/transaction/年/月/日/文件名.扩展名`
  5. 如提供transactionId，系统会自动更新交易记录的imagePath字段
...
| 参数名 | 类型 | 必填 | 描述 |
|-------|------|------|------|
| file | File | 是 | 图标图片文件，仅支持jpg、jpeg、png等图片格式 |
| categoryId | Long | 否 | 分类ID，不传则只上传不关联分类 |
...
```json
{
  ...
  "msg": "图标上传成功",
  "data": {
    "iconUrl": "/profile/category/icon/2023/08/10/icon_20230810120530.jpg"
  }
}
```
...
- **注意事项**:
  1. 文件大小限制为2MB
  2. 仅支持图片格式（jpg、jpeg、png）
  3. 上传的图片会自动生成唯一文件名并保存到系统配置的分类图标路径下
  4. 文件路径格式为：`/profile/category/icon/年/月/日/文件名.扩展名`
  5. 如提供categoryId，系统会自动更新分类记录的icon字段
...
| 参数名 | 类型 | 必填 | 描述 |
|-------|------|------|------|
| userId | Long | 否 | 用户ID（如不提供则使用当前登录用户ID） |
| categoryId | Long | 否 | 分类ID |
| year | Integer | 否 | 年份，如果提供了month则必须同时提供year |
| month | Integer | 否 | 月份（1-12），如果提供则必须同时提供year |
| pageNum | Integer | 否 | 页码，默认1 |
| pageSize | Integer | 否 | 每页大小，默认10 |
...
```bash
curl -X POST "http://example.com/prod-api/finance/transaction/image?transactionId=123" \
  -H "Authorization: Bearer eyJhbGciO..." \
...
```bash
curl -X POST "http://example.com/prod-api/finance/category/icon?categoryId=5" \
  -H "Authorization: Bearer eyJhbGciO..." \
  -H "Content-Type: multipart/form-data" \
